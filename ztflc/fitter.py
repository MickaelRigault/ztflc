...
class DiffImgFitter( object ):
    ...
    def fit(self, **kwargs):
        ...
        # saving results
        self.best_parameters = np.asarray([self.minuit.values[k]
    ...
    def _setup_minuit_(self, step=1, print_level=0, **kwargs):
        ...
        for param in self.FREEPARAMETERS:
            minuit_kwargs[param]           = kwargs.get("%s_guess"%param, 0 if not 'sigma' in param else 1)
            minuit_kwargs["limit_"+param]  = kwargs.get("%s_boundaries"%param, None if not 'sigma' in param else [0,None])
    ...
    def set_data(self, diffimg, psfimg, diffvar=None, shape=None):
        ...
        if np.shape(diffimg) != np.shape(psfimg):
            raise ValueError("diffimg and psfimg do not have the smae shape. They must.")
        ...
        if diffvar is not None:
            if len(np.atleast_1d(diffvar)) == 1:
                ...
                raise ValueError("diffvar is not a single value and diffimg and diffvar do not have the smae shape. ")
        else:
            self.diffvar = 0
    ...
    def get_fit_covmatrix(self):
        ...
        if self._migrad_output_[0]["is_valid"]:
            ...
            fakeMatrix = np.zeros((len(self.best_parameters),len(self.best_parameters)))
    ...
    def show(self, axes=None, res_aspull=True):
        ...
        if res_aspull:
            ...
            norm = 1
            prop = {}
        axr.imshow(((data_-model_)/norm).reshape( self._shape ), origin="lower",**prop)
        if res_aspull:
            axr.text(0.05,0.95, "Pull [-3,3]", color="w", va="top", ha="left",
    ...
    def nfreeparameters(self):
        """ """
        fixed = 0 if not hasattr(self,"_minuit_input") else len([k for k,v in self._minuit_input.items() if "fixed" in k and v])
