...
class DiffImgFitter(object):
    ...
    def show(self, axes=None, res_aspull=True):
        ...
        if res_aspull:
            ...
            norm = 1
            prop = {}
        axr.imshow(((data_-model_)/norm).reshape(self._shape),
                   origin="lower", **prop)
        if res_aspull:
    ...
    def nfreeparameters(self):
        """ """
        fixed = 0 if not hasattr(self, "_minuit_input") else len(
            [k for k, v in self._minuit_input.items() if "fixed" in k and v])
