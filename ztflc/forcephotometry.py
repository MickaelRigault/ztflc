...
class ForcePhotometry():
    ...
    def from_coords(cls, ra, dec, jdmin=None, jdmax=None,  name="unknown"):
        """ Load the ForcePhotometry from Target Name """
        return cls(ZTFTarget.from_coords(ra, dec,
    ...
    def run_forcefit(self, indexes=None, update_diffdata=False,
        ...
        try:
            path_previous_results = os.path.join(LOCALDATA,
                                                 "{}.csv".format(self.io.name))
            previous_results = pandas.read_csv(path_previous_results,
                                               float_precision='round_trip')
        except FileNotFoundError:
            previous_results = pandas.DataFrame(columns=['filename'])

        if verbose:
            print("Starting run_forcefit() for %d image differences"
                  % len(indexes))
        ...
        columns = ['sigma', 'sigma.err', 'ampl', 'ampl.err', 'fval', 'chi2',
                   'chi2dof', 'filename', 'humidity', 'filter', 'obsmjd',
                   'ccdid', 'amp_id', 'gain', 'readnoi', 'darkcur', 'magzp',
                   'magzpunc', 'magzprms', 'clrcoeff', 'clrcounc', 'zpclrcov',
                   'zpmed', 'zpavg', 'zprmsall', 'clrmed', 'clravg', 'clrrms',
                   'qid', 'rcid', 'seeing', 'maglim', 'status', 'filterid',
                   'fieldid', 'moonalt', 'moonillf', 'target_x', 'target_y',
                   'data_hasnan']

        if nprocess > 1:
            dataout_df = pandas.DataFrame(columns=columns)
            bar = ProgressBar(index_count)
            dataout_list = []

            with multiprocessing.Pool(nprocess) as p:
                for j, dataout_i in enumerate(p.imap_unordered(
                    self.get_ith_diffdata_multiprocess, zip(indexes,
                                                            self.filepathes,
                                                            _coords,
                                                            _update_diffdata,
                                                            _no_badsub,
            ...
            for i in indexes:
                ...
                # and will not be fitted again
                # (but it WILL be fitted again if force_refit is True)

                # Parse the query to a dictionary to check
                # if it is 40 characters long or not
                if len(query) == 1:
    ...
    def get_ith_diffdata(self, index, update=False, rebuild=False, **kwargs):
        ...
        update: [bool] -optional-
            if the DiffData is derived (because unknown or rebuild),
            should this be updated to self.diffdata
        ...
        if self.diffdata is None or self.diffdata[index] is None or rebuild:
            diffdata = DiffData(*self.filepathes[index],
                                self.io.get_coordinate(),
                                **kwargs)
            if update:
    ...
    def show_lc(self, ax=None, scalezp=25, reference=False, **kwargs):
        ...
        for i, band_ in [[1, "C0"], [2, "C2"], [3, "C1"]]:
            if i not in self.data_forcefit["filterid"]:
                continue
            flag = np.asarray(self.data_forcefit["filterid"] == i,
                              dtype="bool")
    ...
    def data_forcefit(self):
        ...
        if not hasattr(self, "_data_forcefit"):
            raise AttributeError("data_forcefit not loaded."
                                 + "Run self.run_forcefit()")
